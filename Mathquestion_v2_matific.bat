...
set scriptDir=%~dp0
set dataFile=%scriptDir%wallpaper_path.txt

if exist "%dataFile%" (
    set /p imgPath=<"%dataFile%"
    echo Using previously saved image path: %imgPath%
) else (
...
set shortcutName=WallpaperChanger.lnk
set scriptPath=%~f0

powershell -command "$s=(New-Object -COM WScript.Shell).CreateShortcut('%startupFolder%\%shortcutName%');$s.TargetPath='%scriptPath%';$s.Save()"

echo All advanced methods attempted. Script will now auto-run on startup.
pause
exit /b
